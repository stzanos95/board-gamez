...
class BaseRulesClient(ABC):
    """
    One game's rules, whichever game and wherever they run.
    ...
    `open` and `close` bracket the life of the process. A client is built where
    the process is configured and opened once the event loop is running.
    """

    @abstractmethod
    async def open(self) -> None:
        """
        Make the rules reachable. Called once, after the event loop is up.
        """

    @abstractmethod
    async def close(self) -> None:
        """
        Let calls already in flight finish, then stop.
        """
