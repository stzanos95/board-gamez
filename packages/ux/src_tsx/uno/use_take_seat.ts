import type { UnoSeatChoice, UnoSeatResult } from "@board-gamez/idl/uno/model/table_pb";
...
export type TakeSeatAction = {
  ...
  readonly pendingChoice: UnoSeatChoice | null;
  readonly problem: string | null;
};

/**
 * Take a seat at the UNO table, as one of the choices the game offered.
 *
...
export function useTakeSeat(): TakeSeatAction {
  ...
  const onAnswered = useCallback(
    (result: UnoSeatResult | null, input: TakeSeatInput) => {
      if (result?.table !== undefined) {
        queryClient.setQueryData(unoQueryKeys.table(input.tableId), result.table);
      }
      invalidateAfterWrite(queryClient, tableQueryKeys.detail(input.tableId));
      invalidateAfterWrite(queryClient, tableQueryKeys.list());
      invalidateAfterWrite(queryClient, unoQueryKeys.seatChoices(input.tableId, player.id));
    },
