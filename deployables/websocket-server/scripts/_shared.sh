...
require_docker() {
    if ! command -v docker >/dev/null 2>&1; then
        echo "docker is not installed. To serve without it:" >&2
        echo "  deployables/websocket-server/scripts/local-serve.sh" >&2
        echo "  deployables/websocket-server/scripts/local-test.sh" >&2
        exit 1
    fi
}
