...
from grpc_server.service_host_config import ServiceHostConfig

EXIT_SUCCESS = 0

PROGRAM_NAME = "grpc-server"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        prog=PROGRAM_NAME, description="Serve the board-gamez internal gRPC services."
    )
    parser.add_argument(
        CONFIG_OPTION,
        type=Path,
    ...
    with arguments.config.open(encoding=CONFIG_ENCODING) as config_file:
        config = ServiceHostConfig.from_yaml(config_file.read())
    ServiceHost(config=config).start()
    return EXIT_SUCCESS
